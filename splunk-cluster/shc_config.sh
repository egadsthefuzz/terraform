...
sudo -u splunk echo -e "[general]\nserverName = "$(curl http://169.254.169.254/latest/meta-data/local-hostname)"-sh" > /data/splunk/splunk/etc/system/local/server.conf
sudo -u splunk echo -e "[default]\nhost = "$(curl http://169.254.169.254/latest/meta-data/local-hostname)"-sh" > /data/splunk/splunk/etc/system/local/inputs.conf
...
ixrpeers=""
for i in $(sudo -u splunk /data/splunk/splunk/bin/splunk list search-server -auth admin:${splunkadminpass}|cut -c 16-35|cut -d':' -f 1|sed 's/$/:${splunkingest}/'); do
    ixrpeers+="$i,"
done
ixrpeers=$${ixrpeers:0:-1}
echo $ixrpeers
sudo -u splunk mkdir /data/splunk/splunk/etc/apps/${project_name}
sudo -u splunk mkdir /data/splunk/splunk/etc/apps/${project_name}/default
sudo -u splunk echo -e "[tcpout]\ndefaultGroup = ixrs\n[tcpout:ixrs]\nserver = $ixrpeers\n[indexAndForward]\nindex = false" > /data/splunk/splunk/etc/apps/${project_name}/default/outputs.conf
chmod 774 -R /data/splunk/splunk/etc/apps/${project_name}
service splunk restart
#add indexes.conf